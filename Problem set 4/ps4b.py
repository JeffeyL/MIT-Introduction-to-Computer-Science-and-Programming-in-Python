...
class Message(object):
    ...
    def build_shift_dict(self, shift):
        ...
        punctuation = list(" !@#$%^&*()-_+={}[]|\:;'<>?,./\"")
        dict = {}
        for i, letter in enumerate(lower_letters[:26]):
            dict[letter] = lower_letters[i + shift]
        for i, letter in enumerate(upper_letters[:26]):
            dict[letter] = upper_letters[i + shift]
...
if __name__ == '__main__':
    ...
    plaintext = PlaintextMessage('windows is better than mac', 1)
    print('Expected Output: xjoepxt jt cfuufs uibo nbd')
