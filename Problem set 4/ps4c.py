...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for dic in tranpose_dict_list:
            ...
            decrypt_message_list.append(de_message)
        test = []
        big_test = []
        word_list = self.get_valid_words()
        for mes in decrypt_message_list:
            de_words = mes.split()
        ...
        for tup in big_test:
            if tup[0] == best_choice[0] and tup[1] not in possible_decrypt_message:
                possible_decrypt_message.append(tup[1])
        decrypt_string = ''
        for mes in possible_decrypt_message:
            decrypt_string = decrypt_string + ', ' + mes
        return decrypt_string[1:]


if __name__ == '__main__':

    # TODO: WRITE YOUR TEST CASES HERE
    message = SubMessage('Big Brain')
    permutation = 'iauoe'
    enc_dict = message.build_transpose_dict(permutation)
    print("Original message:", message.get_message_text(), "Permutation:", permutation)
    print("Expected encryption:", "Bug Bruin")
